...
(function (root, factory) {
  ...
  // Softened from 4 (playtesting: "cool but way too hard") — at the old rate
  // a guess just 25% of the axis range off in each direction already scored
  // ~368/1000. 2.5 keeps a near-exact guess close to the same near-1000
  // score while giving a decent-but-imperfect guess meaningfully more credit,
  // without flattening the curve so much that wild guesses stop being
  // punished (see the exact-formula regression test in test/pure.test.js).
  const SCORE_DECAY_RATE = 2.5;
  ...
  const PACKS = {
    ...
    mlb_pitchers: {
      ...
      article: 'an',
      emoji: '⚾',
      file: 'mlb_pitchers.json',
      defaultPair: ['era', 'strikeouts'],
      statDefs: {
        // Unlike batting average/OBP/slugging (naturally 0-1, needs whole-
        // number scaling per the schema rule in CLAUDE.md), none of era/
        // whip/k9 need it: ERA (~1.5-5), WHIP (~0.9-1.5), and K/9 (~4-13) are
        // all already several tenths-to-units wide across this pool, not a
        // narrow sliver that floor/ceil would collapse — stored as plain
        // decimals, which also keeps them reading like real WHIP/ERA figures
        // instead of an unfamiliar scaled integer.
        era: { label: 'Career ERA', short: 'ERA' },
        whip: { label: 'Career WHIP', short: 'WHIP' },
        k9: { label: 'Strikeouts per 9 IP', short: 'K/9' },
        wins: { label: 'Career Wins', short: 'W' },
        strikeouts: { label: 'Career Strikeouts', short: 'K' },
        // Legitimately 0 for most starters — a real value (no save
        // opportunities), never omitted, same principle as coastline: 0 for
        // a landlocked country.
        saves: { label: 'Career Saves', short: 'SV' },
      },
    },
    nhl: {
      label: 'NHL',
  ...
  // Same rejection-sampling idea as pickEligiblePair, but scoped to ONE
  // specific entry rather than "does any entry in the pack have this pair" —
  // needed when Kitchen Prep forces a specific entry without also locking the
  // stat pair. pickEligiblePair alone isn't enough there: it can return a pair
  // some OTHER entry in the pack has, which the forced entry itself lacks,
  // silently swapping in a random different entry at pickEntry() time while
  // the UI still shows the originally forced one (see CLAUDE.md).
  function pickEligiblePairForEntry(entry, keys, avoid) {
    const preferred = avoid ? keys.filter((k) => !avoid.includes(k)) : keys;
    const pool = preferred.length >= 2 ? preferred : keys;
    for (let i = 0; i < 200; i++) {
      const x = randomKey(pool);
      const y = randomKey(pool, x);
      if (Number.isFinite(entry[x]) && Number.isFinite(entry[y])) return { x, y };
    }
    return { x: keys[0], y: keys[1] };
  }
  ...
  // Fisher-Yates sample of up to `count` OTHER entries from `pool` (excluding
  // `target` by reference) — used to plot real reference points on the grid in
  // Regular difficulty (see CLAUDE.md). Naturally returns fewer than `count` if
  // the pool doesn't have enough (e.g. a small pack) rather than crashing —
  // same graceful-degradation approach as pickEligiblePair's bounded retry.
  function pickReferenceEntries(pool, target, count) {
